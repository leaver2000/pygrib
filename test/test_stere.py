...
grbs = pygrib.open(sample_dir("ngm.grb"))
...
pj = ccrs.Stereographic(
    globe=globe,
    central_longitude=grb.projparams["lon_0"],
...
def test_stere1():
    ...
    coords = pj.transform_points(
        ccrs.PlateCarree(),
        np.asarray([lons[0, 0], lons[-1, -1]]),
        np.asarray([lats[0, 0], lats[-1, -1]]),
    )
...
grbs = pygrib.open(sample_dir("CMC_reg_WIND_ISBL_300_ps60km_2010052400_P012.grib"))
...
globe = ccrs.Globe(
    ellipse="sphere",
    semimajor_axis=grb2.projparams["a"],
    semiminor_axis=grb2.projparams["b"],
)
pj2 = ccrs.Stereographic(
    globe=globe,
    central_longitude=grb2.projparams["lon_0"],
...
def test_stere2():
    ...
    coords = pj2.transform_points(
        ccrs.PlateCarree(),
        np.asarray([lons2[0, 0], lons2[-1, -1]]),
        np.asarray([lats2[0, 0], lats2[-1, -1]]),
    )
